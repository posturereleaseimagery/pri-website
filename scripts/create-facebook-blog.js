...
// Read Facebook posts and extract content
const facebookDataPath = path.join(process.cwd(), 'public/facebook/posts/profile_posts_1.json');
const facebookPosts = JSON.parse(fs.readFileSync(facebookDataPath, 'utf-8'));

// Extract posts with substantial content
const extractedPosts = facebookPosts.filter(post => {
  return post.data && post.data.some(item => item.post && item.post.trim().length > 50);
}).map((post, index) => {
  const contentItem = post.data.find(item => item.post);
  ...
  if (post.attachments && post.attachments.length > 0) {
    post.attachments.forEach(attachment => {
      if (attachment.data && attachment.data.length > 0) {
        attachment.data.forEach(dataItem => {
...
const blogPosts = extractedPosts.map(post => {
...
// Generate blog content file
const articles = blogPosts.filter(p => p.type === 'article');
const tidbits = blogPosts.filter(p => p.type === 'tidbit');
...
${tidbits.map(tidbit => `  {
    ...
    tags: [${tidbit.tags.map(tag => `"${tag}"`).join(', ')}]
  }`).join(',\n')}
];
`;
...
articles.forEach((article, i) => {
  ...
  console.log(`Media files: ${article.mediaFiles.length}`);
  if (article.mediaFiles.length > 0) {
    article.mediaFiles.forEach(file => console.log(`  - ${file}`));
  }
  console.log(`Excerpt: ${article.excerpt}`);
});

console.log('\n=== TIDBITS ===');
tidbits.forEach((tidbit, i) => {
  console.log(`\n--- Tidbit ${i + 1} ---`);
  console.log(`Date: ${tidbit.publishDate}`);
  console.log(`Tags: ${tidbit.tags.join(', ')}`);
  console.log(`Media files: ${tidbit.mediaFiles.length}`);
  if (tidbit.mediaFiles.length > 0) {
    tidbit.mediaFiles.forEach(file => console.log(`  - ${file}`));
  }
  console.log(`Text: ${tidbit.text.substring(0, 150)}...`);
});
